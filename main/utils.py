...
def make_confusion_matrix(model, dataloader, n_classes):
...
def plot_confusion_matrix(confusion_matrix, classes, save_path=None):
    fig, ax = plt.subplots(figsize=(len(classes) + 2, len(classes) + 2))
    sn.set(font_scale=1.4)  # Set the font scale for the heatmap
